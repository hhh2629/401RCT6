...
#define btn_dowm_port GPIOB
#define btn_dowm_pin  GPIO_PIN_15

static Button btn_up;
static Button btn_enter;
static Button btn_dowm;
...
uint8_t read_button_gpio(uint8_t button_id)
{
    switch (button_id) {
        ...
        case 2:
            return HAL_GPIO_ReadPin(btn_dowm_port, btn_dowm_pin);
        default:
            return 0;
    }
}
...
void Btn_single_click_handler(Button* btn)
{
    ...
    {
        case 0://W25Q32_Show_On_LCD_DMA(0,0,320,240,0x0);
                T=1;
                printf_DMA("key0");            
            break;
        case 1://W25Q32_Show_On_LCD_DMA(0,0,179,240,0x26000); 
                T=2;
                printf_DMA("key1");
            break;
        case 2://W25Q32_Show_On_LCD_DMA(0,0,320,205,0x3B000); 
                T=3;
                printf_DMA("key2");
            break;
        default:
...
void Btn_double_click_handler(Button* btn)
{
    switch(btn->button_id)
    {
        case 0:
...
void Btn_long_click_handler(Button* btn)
{
    switch(btn->button_id)
    {
        case 0:
...
void KEY_Init()
{
    ...
    button_init(&btn_up,    read_button_gpio, 0, 0);
    button_init(&btn_enter, read_button_gpio, 0, 1);
    button_init(&btn_dowm,  read_button_gpio, 0, 2);
    
    button_attach(&btn_up,    BTN_SINGLE_CLICK, Btn_single_click_handler);
    button_attach(&btn_enter, BTN_SINGLE_CLICK, Btn_single_click_handler);
    button_attach(&btn_dowm,  BTN_SINGLE_CLICK, Btn_single_click_handler);
    
    button_attach(&btn_up,    BTN_DOUBLE_CLICK, Btn_double_click_handler);
    button_attach(&btn_enter, BTN_DOUBLE_CLICK, Btn_double_click_handler);
    button_attach(&btn_dowm,  BTN_DOUBLE_CLICK, Btn_double_click_handler);
    
    button_attach(&btn_up,    BTN_LONG_PRESS_START, Btn_long_click_handler);
    button_attach(&btn_enter, BTN_LONG_PRESS_START, Btn_long_click_handler);
    button_attach(&btn_dowm,  BTN_LONG_PRESS_START, Btn_long_click_handler);
    
    button_start(&btn_up);
    button_start(&btn_enter);
    button_start(&btn_dowm);
}
