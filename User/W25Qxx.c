...
{
    W25Qxx_CS_Low();
    W25Qxx_SPI_RW_Byte(W25Qxx_CMD_Read_Data); // 发送读取命令
    ...
    while (remaining > 0)
    {
        uint16_t read_len = (remaining >= 256) ? 256 : (uint16_t)remaining;
        // 使用DMA连续接收数据（替换原有单字节读取循环）
        HAL_SPI_Receive(&W25Qxx_SPI_Handle, current_buf, read_len, HAL_MAX_DELAY);
